...
static int yielding_handler(rpc_ctx *ctx, const rpc_value *args, size_t argc, rpc_writer *out, void *user_data) {
  int *seen = user_data;
  assert(argc == 1);
...
int main(void) {
  ...
  assert(rpc_scheduler_submit(scheduler, 7, 9, yielding_handler, &seen, payload.data, payload.len, on_done, NULL) == 0);
