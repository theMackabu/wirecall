...
static inline int rpc_fixed_arena_init(rpc_fixed_arena *arena, size_t elem_size,
                                       size_t capacity) {
  memset(arena, 0, sizeof(*arena));
  arena->elem_size = rpc_arena_align(elem_size < sizeof(void *) ? sizeof(void *) : elem_size);
  arena->capacity = capacity;
  arena->bytes = arena->elem_size * capacity;
  arena->base = mmap(NULL, arena->bytes, PROT_READ | PROT_WRITE,
