...
#if defined(__GNUC__) || defined(__clang__)
  return RPC_TRACE_UNLIKELY(__atomic_load_n(&rpc_trace_enabled, __ATOMIC_RELAXED));
#else
...
static inline void rpc_trace_end(rpc_trace_metric metric, uint64_t start_ns) {
  if (rpc_trace_is_enabled() && start_ns != 0) { rpc_trace_end_slow(metric, start_ns); }
}

static inline void rpc_trace_add(rpc_trace_metric metric, uint64_t value) {
  if (rpc_trace_is_enabled()) { rpc_trace_add_slow(metric, value); }
}

static inline void rpc_trace_worker_end(uint32_t worker, rpc_trace_worker_metric metric, uint64_t start_ns) {
  if (rpc_trace_is_enabled() && start_ns != 0) { rpc_trace_worker_end_slow(worker, metric, start_ns); }
}

static inline void rpc_trace_worker_add(uint32_t worker, rpc_trace_worker_metric metric, uint64_t value) {
  if (rpc_trace_is_enabled()) { rpc_trace_worker_add_slow(worker, metric, value); }
}
