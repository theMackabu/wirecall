...
static int apply_filter(rpc_backend *backend, int fd, int16_t filter,
                        uint16_t flags, uintptr_t user) {
  struct kevent change;
  EV_SET(&change, (uintptr_t)fd, filter, flags, 0, 0, (void *)user);
  int rc = kevent(backend->kq, &change, 1, NULL, 0, NULL);
  if (rc != 0 && errno == ENOENT && (flags & EV_DELETE)) {
    return 0;
  }
  return rc;
}

static int set_interest(rpc_backend *backend, int fd, uint32_t events,
                        uintptr_t user) {
  int rc = 0;
...
int rpc_backend_poll(rpc_backend *backend, rpc_backend_event *events, int max_events,
                     int timeout_ms) {
  if (!backend || !events || max_events <= 0) {
    return -1;
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (kev[i].filter == EVFILT_USER) {
      events[i].events |= RPC_BACKEND_WAKE;
    }
    if (kev[i].filter == EVFILT_READ) {
      events[i].events |= RPC_BACKEND_READ;
    }
    if (kev[i].filter == EVFILT_WRITE) {
      events[i].events |= RPC_BACKEND_WRITE;
    }
  }
  return n;
}
